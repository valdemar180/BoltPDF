...
export const Header: React.FC = () => {
  return (
    <header className="w-full border-b border-gray-800 bg-background/50 backdrop-blur-md px-6 py-4 flex justify-between items-center fixed top-0 left-0 z-50">
      ...
      >
        {/* Ícone de Raio com efeito Glow (URI corrigida conforme especificação oficial do W3C) */}
        <div className="p-2 bg-neonCyan/10 rounded-lg border border-neonCyan/30 group-hover:border-neonCyan transition-all shadow-neon-cyan">
          <svg xmlns="http://w3.org" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 text-neonCyan animate-pulse" aria-hidden="true">
      ...
      <div className="text-sm text-gray-400 bg-surface/50 px-4 py-2 rounded-full border border-gray-800 font-mono flex items-center gap-1.5">
        <span aria-hidden="true">⚡</span> Client-Side Engine v1.0
      </div>
    </header>
  );
};
